...
def makeUrl(search, start_pg, end_pg, s_date, e_date):
    ...
    e_from = e_date.replace(".", "")
    if start_pg == end_pg:
        start_page = makePgNum(start_pg)
        url = 'https://search.naver.com/search.naver?where=news&query=' + search + '&sm=tab_opt&sort=0&photo=0&field=0&pd=3&ds=' + s_date + '&de=' + e_date + '&docid=&related=0&mynews=0&office_type=0&office_section_code=0&news_office_checked=&nso=so%3Ar%2Cp%3Afrom' + s_from + 'to' + e_from + '&is_sug_officeid=0' + "&start=" + str(
            start_page)
        print("생성url: ", url)
        return url
    else:
